...
static uint8_t eth_type_is_ip_and_my_ip(uint16_t len) {
  return len >= 42 && gPB[ETH_TYPE_H_P] == ETHTYPE_IP_H_V &&
                      ...
                      (memcmp(gPB + IP_DST_P, EtherCard::myip, 4) == 0 || memcmp(gPB + IP_DST_P, allOnes, 4) == 0);
                      //TODO: hack to make broadcast IP work, make it nicer?
}
...
void EtherCard::clientIcmpRequest(const uint8_t *destip) {
  ...
  gPB[ICMP_IDENT_L_P] = EtherCard::myip[3]; // last uint8_t of my IP
...
void EtherCard::sendUdp (char *data,uint16_t datalen,uint16_t sport, uint8_t *dip, uint16_t dport) {
...
void EtherCard::browseUrl (prog_char *urlbuf, const char *urlbuf_varpart, prog_char *hoststr, prog_char *additionalheaderline, void (*callback)(uint8_t,uint16_t,uint16_t)) {
